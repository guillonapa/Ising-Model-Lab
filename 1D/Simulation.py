...
# 25 -- int(5.0e7)
# 100 -- int(1.8e8)
MCS = int(2.0e5) #  #8) # 7 # number of Monte Carlo steps
...
for KbT in KbT_vals:
	...
	while i < MCS:
		...
		evt.append(float(current_energy) / NUM_SPINS)
		mvt.append(float(current_abs_mag) / NUM_SPINS)
	...
	if KbT < 1.35:
		evt_fig = plt.figure()
		plt.plot(evt)
		evt_fig.suptitle('E vs. MCS', fontsize=20)
		plt.xlabel('MCS', fontsize=18)
		plt.ylabel('E (J / N)', fontsize=16)
		evt_fig.savefig('plots/evt.jpg')
		plt.close()
	
		mvt_fig = plt.figure()
		plt.plot(mvt)
		mvt_fig.suptitle('M vs. MCS', fontsize=20)
		plt.xlabel('MCS', fontsize=18)
		plt.ylabel('M (mu / N)', fontsize=16)
		mvt_fig.savefig('plots/mvt.jpg')
		plt.close()
...
#	print '#######'
	energy_list.append(float(energy_avg) / NUM_SPINS)
	spec_heat = energy_var / (KbT * KbT) # get_spec_heat(energy_list) / (T*T) TODO: scale by Kb?
	spec_heat_list.append(float(spec_heat) / NUM_SPINS)
	
	mag_list.append(float(mag_avg) / NUM_SPINS)
	susceptibility = mag_var / KbT # TODO: scale by Kb?
	susceptibility_list.append(float(susceptibility) / NUM_SPINS)
